...
namespace Food_Haven.UnitTest.Admin_UpdateStoreIsActive_Test
{
    public class UpdateStoreIsActive_Test
    {
        ...
        [Test]
        public async Task UpdateStoreIsActive_StoreExists_ReturnsSuccessJson()
        {
            var storeId = Guid.NewGuid();
            bool isActive = true;
            _storeServiceMock.Setup(s => s.UpdateStoreIsActiveAsync(storeId, isActive)).ReturnsAsync(true);

            var result = await _controller.UpdateStoreIsActive(storeId, isActive) as JsonResult;
            Assert.IsNotNull(result);

            var value = result.Value;
            var type = value.GetType();
            Assert.IsTrue((bool)type.GetProperty("success")?.GetValue(value));
            Assert.AreEqual("Store status updated successfully", type.GetProperty("message")?.GetValue(value));
            Assert.AreEqual(isActive, type.GetProperty("isActive")?.GetValue(value));
        }

        [Test]
        public async Task UpdateStoreIsActive_StoreExistsFalse_ReturnsSuccessJson()
        {
            var storeId = Guid.NewGuid();
            bool isActive = false;
            _storeServiceMock.Setup(s => s.UpdateStoreIsActiveAsync(storeId, isActive)).ReturnsAsync(true);
        ...
        [Test]
        public async Task UpdateStoreIsActive_StoreNotFound_ReturnsFailureJson()
        {
            var storeId = Guid.NewGuid();
            bool isActive = false;
            _storeServiceMock.Setup(s => s.UpdateStoreIsActiveAsync(storeId, isActive)).ReturnsAsync(false);
            ...
            var value = result.Value;
            var type = value.GetType();
            Assert.IsFalse((bool)type.GetProperty("success")?.GetValue(value));
            Assert.AreEqual("Store not found", type.GetProperty("message")?.GetValue(value));
        }
    }
}
