...
namespace Food_Haven.UnitTest.User_ViewRecipe_Test
    {
        public class ViewRecipe
        {
         ...
         private ManageTransaction _manageTransaction; // Real instance
            ...
            [Test]
            public async Task ViewRecipe_CanConnectWithServer_ReturnsViewWithData()
            {
                // Arrange
                var recipes = new List<Recipe>
                {
                    new Recipe { ID = Guid.NewGuid(), IsActive = true, status = "Accept", TypeOfDishID = Guid.NewGuid() }
                };
                var typeOfDishes = new List<TypeOfDish>
                {
                    new TypeOfDish { ID = recipes[0].TypeOfDishID, Name = "Type1", IsActive = true }
                };

                _recipeServiceMock.Setup(r => r.ListAsync()).ReturnsAsync(recipes);
                _typeOfDishServiceMock.Setup(t => t.ListAsync()).ReturnsAsync(typeOfDishes);

                // Act
                var result = await _controller.ViewRecipe(null);

                // Assert
                var viewResult = result as ViewResult;
                Assert.IsNotNull(viewResult);
                var model = viewResult.Model as MyViewRecipePageViewModel;
                Assert.IsNotNull(model);
                Assert.IsTrue(model.Recipes.Any());
            }

            [Test]
            public async Task ViewRecipe_CanConnectWithServer_ReturnsViewWithEmptyList()
            {
                // Arrange
                _recipeServiceMock.Setup(r => r.ListAsync()).ReturnsAsync(new List<Recipe>());
                _typeOfDishServiceMock.Setup(t => t.ListAsync()).ReturnsAsync(new List<TypeOfDish>());
                ...
                var model = viewResult.Model as MyViewRecipePageViewModel;
                Assert.IsNotNull(model);
                Assert.IsFalse(model.Recipes.Any());
            }

            [Test]
            public async Task ViewRecipe_CannotConnectWithServer_ReturnsErrorView()
            {
                // Arrange
                _recipeServiceMock.Setup(r => r.ListAsync()).ThrowsAsync(new Exception("An unknown error occurred..."));
                _typeOfDishServiceMock.Setup(t => t.ListAsync()).ThrowsAsync(new Exception("An unknown error occurred..."));
                ...
                StringAssert.Contains("An unknown error occurred", statusResult.Value.ToString());
            }
        }
    }
