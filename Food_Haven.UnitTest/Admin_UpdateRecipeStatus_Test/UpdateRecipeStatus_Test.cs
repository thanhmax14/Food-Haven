...
namespace Food_Haven.UnitTest.Admin_UpdateRecipeStatus_Test
{
    public class UpdateRecipeStatus_Test
    {
        ...
        [Test]
        public async Task UpdateRecipeStatus_ReturnsJsonResult_WhenApproved()
        {
            // Arrange
            var admin = new AppUser { UserName = "admin", Email = "admin@site.com" };
            var recipeId = Guid.NewGuid();
            var recipe = new Recipe { ID = recipeId, status = "Pending" };
            ...
            _recipeServiceMock.Setup(x => x.FindAsync(It.IsAny<System.Linq.Expressions.Expression<Func<Recipe, bool>>>())).ReturnsAsync(recipe);
            _recipeServiceMock.Setup(x => x.UpdateAsync(recipe)).Returns(Task.CompletedTask);
            _recipeServiceMock.Setup(x => x.SaveChangesAsync()).ReturnsAsync(1);

            // Act
            var result = await _controller.UpdateRecipeStatus(recipeId, "Approved", null);
            ...
            Assert.IsTrue((bool)valueType.GetProperty("success").GetValue(value));
            Assert.AreEqual("Recipe approved successfully.", valueType.GetProperty("message").GetValue(value));
            Assert.AreEqual("Accept", recipe.status);
        }

        [Test]
        public async Task UpdateRecipeStatus_ReturnsJsonResult_WhenRejected()
        {
            // Arrange
            var admin = new AppUser { UserName = "admin", Email = "admin@site.com" };
            var recipeId = Guid.NewGuid();
            var recipe = new Recipe { ID = recipeId, status = "Pending" };
            string rejectNote = "Not enough details";
            _userManagerMock.Setup(x => x.GetUserAsync(It.IsAny<System.Security.Claims.ClaimsPrincipal>())).ReturnsAsync(admin);
            _userManagerMock.Setup(x => x.IsInRoleAsync(admin, "Admin")).ReturnsAsync(true);
            _recipeServiceMock.Setup(x => x.FindAsync(It.IsAny<System.Linq.Expressions.Expression<Func<Recipe, bool>>>())).ReturnsAsync(recipe);
            _recipeServiceMock.Setup(x => x.UpdateAsync(recipe)).Returns(Task.CompletedTask);
            ...
            // Act
            var result = await _controller.UpdateRecipeStatus(recipeId, "Rejected", rejectNote);
            ...
            var valueType = value.GetType();
            Assert.IsTrue((bool)valueType.GetProperty("success").GetValue(value));
            Assert.AreEqual("Recipe rejected successfully.", valueType.GetProperty("message").GetValue(value));
            Assert.AreEqual("Reject", recipe.status);
            Assert.AreEqual(rejectNote, recipe.RejectNote);
        }

       

        [Test]
        public async Task UpdateRecipeStatus_ReturnsNotFound_WhenRecipeNotFound()
        {
            // Arrange
            var admin = new AppUser { UserName = "admin", Email = "admin@site.com" };
            var recipeId = Guid.NewGuid();
            _userManagerMock.Setup(x => x.GetUserAsync(It.IsAny<System.Security.Claims.ClaimsPrincipal>())).ReturnsAsync(admin);
            _userManagerMock.Setup(x => x.IsInRoleAsync(admin, "Admin")).ReturnsAsync(true);
            _recipeServiceMock.Setup(x => x.FindAsync(It.IsAny<System.Linq.Expressions.Expression<Func<Recipe, bool>>>())).ReturnsAsync((Recipe)null);
