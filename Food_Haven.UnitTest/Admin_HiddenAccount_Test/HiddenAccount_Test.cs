...
namespace Food_Haven.UnitTest.Admin_HiddenAccount_Test
{
    [TestFixture]
    public class HiddenAccount_Test
    {
        ...
        [Test]
        public async Task HiddenAccount_UserNotFound_ReturnsJsonError()
        {
            ...
            _userManagerMock.Setup(x => x.FindByEmailAsync("user@email.com")).ReturnsAsync((AppUser)null);

            var result = await _controller.HiddenAccount(new UsersViewModel { Email = "user@email.com" }) as JsonResult;
            Assert.IsNotNull(result);
            var value = result.Value;
            var type = value.GetType();
            Assert.IsFalse((bool)type.GetProperty("success")?.GetValue(value));
            Assert.AreEqual("User not found", type.GetProperty("message")?.GetValue(value));
        }

    
        [Test]
        public async Task HiddenAccount_Success_ReturnsJsonSuccess()
        {
            var admin = new AppUser { UserName = "admin" };
            var user = new AppUser { Email = "user@email.com" };
            _userManagerMock.Setup(x => x.GetUserAsync(It.IsAny<System.Security.Claims.ClaimsPrincipal>())).ReturnsAsync(admin);
            _userManagerMock.Setup(x => x.IsInRoleAsync(admin, "Admin")).ReturnsAsync(true);
            _userManagerMock.Setup(x => x.FindByEmailAsync("user@email.com")).ReturnsAsync(user);
            ...
            Assert.AreEqual("User account Banned successfully", type.GetProperty("message")?.GetValue(value));
        }
    }
}
