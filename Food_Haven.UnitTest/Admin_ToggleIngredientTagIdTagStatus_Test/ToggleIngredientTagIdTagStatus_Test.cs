...
namespace Food_Haven.UnitTest.Admin_ToggleIngredientTagIdTagStatus_Test
{
    public class ToggleIngredientTagIdTagStatus_Test
    {
        ...
        [Test]
        public async Task ToggleIngredientTagStatus_ReturnsJsonTrue_WhenServiceReturnsTrue()
        {
            // Arrange
            var id = Guid.NewGuid();
            bool isActive = true;
            _ingredientTagServiceMock.Setup(x => x.ToggleIngredientTagStatus(id, isActive)).ReturnsAsync(true);

            // Act
            var result = await _controller.ToggleIngredientTagStatus(id, isActive);

            // Assert
            var jsonResult = result as JsonResult;
            Assert.That(jsonResult, Is.Not.Null);
            // Fix: Access the anonymous object's property via reflection
            var success = jsonResult.Value.GetType().GetProperty("success")?.GetValue(jsonResult.Value);
            Assert.That(success, Is.True);
        }

        [Test]
        public async Task ToggleIngredientTagStatus_ReturnsJsonFalse_WhenServiceReturnsFalse()
        {
            // Arrange
            var id = Guid.NewGuid();
            bool isActive = false;
            _ingredientTagServiceMock.Setup(x => x.ToggleIngredientTagStatus(id, isActive)).ReturnsAsync(false);
        ...
        [Test]
        public async Task ToggleIngredientTagStatus_ReturnsJsonFalse_WhenIdIsInvalid()
        {
            // Arrange
            var invalidId = Guid.NewGuid(); // Giả sử id này không tồn tại
            bool isActive = true;
            _ingredientTagServiceMock.Setup(x => x.ToggleIngredientTagStatus(invalidId, isActive)).ReturnsAsync(false);

            // Act
            var result = await _controller.ToggleIngredientTagStatus(invalidId, isActive);
