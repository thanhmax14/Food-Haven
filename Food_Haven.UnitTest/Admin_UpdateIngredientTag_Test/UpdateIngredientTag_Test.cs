...
namespace Food_Haven.UnitTest.Admin_UpdateIngredientTag_Test
{
    public class UpdateIngredientTag_Test
    {
        ...
        [Test]
        public async Task UpdateIngredientTag_Get_ReturnsNotFound_WhenEntityDoesNotExist()
        {
            // Arrange
            var id = Guid.NewGuid();
            _ingredientTagServiceMock.Setup(x => x.GetAsyncById(id)).ReturnsAsync((IngredientTag)null);

            // Act
            var result = await _controller.UpdateIngredientTag(id);
        ...
        [Test]
        public async Task UpdateIngredientTag_Post_ReturnsViewWithError_WhenNameExists()
        {
            // Arrange
            var model = new IngredientTagUpdateViewModel { ID = Guid.NewGuid(), Name = "Test", IsActive = true };
            _ingredientTagServiceMock.Setup(x => x.ExistsAsync(model.Name, model.ID)).ReturnsAsync(true);

            // Act
            var result = await _controller.UpdateIngredientTag(model);

            // Assert
            var viewResult = result as ViewResult;
            Assert.That(viewResult, Is.Not.Null);
            Assert.That(_controller.TempData["SwalError"], Is.EqualTo("The ingredient tag name already exists."));
        ...
        [Test]
        public async Task UpdateIngredientTag_Post_RedirectsAndSetsSuccessMessage_WhenUpdateIsSuccessful()
        {
            // Arrange
            var model = new IngredientTagUpdateViewModel { ID = Guid.NewGuid(), Name = "Test", IsActive = true };
            var entity = new IngredientTag { ID = model.ID, Name = model.Name, IsActive = model.IsActive };
            _ingredientTagServiceMock.Setup(x => x.ExistsAsync(model.Name, model.ID)).ReturnsAsync(false);
            _ingredientTagServiceMock.Setup(x => x.GetAsyncById(model.ID)).ReturnsAsync(entity);
            _ingredientTagServiceMock.Setup(x => x.UpdateAsync(entity)).Returns(Task.CompletedTask);
            _ingredientTagServiceMock.Setup(x => x.SaveChangesAsync()).ReturnsAsync(1);
            ...
            Assert.That(redirectResult.ActionName, Is.EqualTo("GetAllIngredientTag"));
            Assert.That(_controller.TempData["SuccessMessage"], Is.EqualTo("Ingredient tag has been updated successfully!"));
        }
