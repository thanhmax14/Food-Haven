...
namespace Food_Haven.UnitTest.Admin_UpdateCategory_Test
{
    public class UpdateCategory_Test
    {
        ...
        [Test]
        public void UpdateCategory_ModelStateInvalid_ReturnsViewWithModel()
        {
            var model = new CategoryUpdateViewModel { ID = Guid.NewGuid(), Commission = 10 };
            _controller.ModelState.AddModelError("Commission", "Required");

            var result = _controller.UpdateCategory(model) as ViewResult;
            Assert.IsNotNull(result);
            Assert.AreEqual(model, result.Model);
        }

        [Test]
        public void UpdateCategory_Success_ReturnsViewWithUpdatedModel()
        {
            var model = new CategoryUpdateViewModel { ID = Guid.NewGuid(), Commission = 10 };
            var updatedModel = new CategoryUpdateViewModel { ID = model.ID, Commission = 20 };

            _categoryServiceMock.Setup(x => x.UpdateCategory(model));
            _categoryServiceMock.Setup(x => x.GetCategoryForUpdate(model.ID)).Returns(updatedModel);

            var result = _controller.UpdateCategory(model) as ViewResult;
            Assert.IsNotNull(result);
            Assert.AreEqual(updatedModel, result.Model);
            Assert.AreEqual("Category updated successfully!", _controller.ViewBag.SuccessMessage);
        }

        [Test]
        public void UpdateCategory_NameExists_ReturnsViewWithModelError()
        {
            var model = new CategoryUpdateViewModel { ID = Guid.NewGuid(), Name = "ExistingName", Commission = 10 };
            _categoryServiceMock.Setup(x => x.UpdateCategory(model)).Throws(new Exception("Name already exists."));
        ...
        [Test]
        public void UpdateCategory_DisplayOrderExists_ReturnsViewWithModelError()
        {
            var model = new CategoryUpdateViewModel { ID = Guid.NewGuid(), Name = "Test", Commission = 10 };
            // Giả lập lỗi Display Order đã tồn tại
            _categoryServiceMock.Setup(x => x.UpdateCategory(model)).Throws(new Exception("Display Order already exists."));
        ...
        [Test]
        public void UpdateCategory_DisplayOrderIsZero_ReturnsViewWithModelError()
        {
            var model = new CategoryUpdateViewModel { ID = Guid.NewGuid(), Name = "Test", Commission = 10 };
            ...
            Assert.IsNotNull(result);
            Assert.AreEqual(model, result.Model);
            Assert.IsTrue(_controller.ModelState.ContainsKey("Number"));
        }

        [Test]
        public void UpdateCategory_CommissionIsOutOfRange_ReturnsViewWithModelError()
        {
            var model = new CategoryUpdateViewModel { ID = Guid.NewGuid(), Name = "Test", Commission = 101 };
            _controller.ModelState.AddModelError("Commission", "Commission must be between 0 and 100.");

            var result = _controller.UpdateCategory(model) as ViewResult;
            Assert.IsNotNull(result);
            Assert.AreEqual(model, result.Model);
            Assert.IsTrue(_controller.ModelState.ContainsKey("Commission"));
        }
    }
}
