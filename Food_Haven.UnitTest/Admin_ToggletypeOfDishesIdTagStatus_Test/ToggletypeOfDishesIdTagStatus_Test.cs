...
namespace Food_Haven.UnitTest.Admin_ToggletypeOfDishesIdTagStatus_Test
{
    public class ToggletypeOfDishesIdTagStatus_Test
    {
        ...
        [Test]
        public async Task ToggletypeOfDishesIdTagStatus_ReturnsJsonResult_WhenSuccessTrue()
        {
            var id = Guid.NewGuid();
            bool isActive = true;
            _typeOfDishServiceMock.Setup(x => x.ToggletypeOfDishesIdTagStatus(id, isActive)).ReturnsAsync(true);

            var result = await _controller.ToggletypeOfDishesIdTagStatus(id, isActive);

            Assert.That(result, Is.InstanceOf<JsonResult>());
            var jsonResult = result as JsonResult;
            var value = jsonResult.Value;
            var successProperty = value.GetType().GetProperty("success");
            Assert.IsNotNull(successProperty, "JsonResult.Value does not contain a 'success' property");
            var successValue = (bool)successProperty.GetValue(value);
            Assert.IsTrue(successValue);
        }

        [Test]
        public async Task ToggletypeOfDishesIdTagStatus_ReturnsJsonResult_WhenSuccessFalse()
        {
            var id = Guid.NewGuid();
            bool isActive = false;
            _typeOfDishServiceMock.Setup(x => x.ToggletypeOfDishesIdTagStatus(id, isActive)).ReturnsAsync(false);
