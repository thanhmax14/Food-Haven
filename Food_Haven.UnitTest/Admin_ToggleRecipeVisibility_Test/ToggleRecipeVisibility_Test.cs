...
namespace Food_Haven.UnitTest.Admin_ToggleRecipeVisibility_Test
{
    public class ToggleRecipeVisibility_Test
    {
        ...
        [Test]
        public async Task ToggleRecipeVisibility_ReturnsJsonResult_WhenShow()
        {
            // Arrange
            var admin = new AppUser { UserName = "admin", Email = "admin@site.com" };
            var recipeId = Guid.NewGuid();
            var recipe = new Recipe { ID = recipeId, IsActive = false };
            ...
            // Act
            var result = await _controller.ToggleRecipeVisibility(recipeId, "Show");
            ...
            Assert.AreEqual("Recipe has been shown successfully.", valueType.GetProperty("message").GetValue(value));
            Assert.IsTrue(recipe.IsActive);
        }

        [Test]
        public async Task ToggleRecipeVisibility_ReturnsJsonResult_WhenHide()
        {
            // Arrange
            var admin = new AppUser { UserName = "admin", Email = "admin@site.com" };
            var recipeId = Guid.NewGuid();
            var recipe = new Recipe { ID = recipeId, IsActive = true };
            _userManagerMock.Setup(x => x.GetUserAsync(It.IsAny<System.Security.Claims.ClaimsPrincipal>())).ReturnsAsync(admin);
            _userManagerMock.Setup(x => x.IsInRoleAsync(admin, "Admin")).ReturnsAsync(true);
            _recipeServiceMock.Setup(x => x.FindAsync(It.IsAny<System.Linq.Expressions.Expression<Func<Recipe, bool>>>())).ReturnsAsync(recipe);
            _recipeServiceMock.Setup(x => x.UpdateAsync(recipe)).Returns(Task.CompletedTask);
            ...
            // Act
            var result = await _controller.ToggleRecipeVisibility(recipeId, "Hide");
            ...
            var valueType = value.GetType();
            Assert.IsTrue((bool)valueType.GetProperty("success").GetValue(value));
            Assert.AreEqual("Recipe has been hidden successfully.", valueType.GetProperty("message").GetValue(value));
            Assert.IsFalse(recipe.IsActive);
        }

        [Test]
        public async Task ToggleRecipeVisibility_ReturnsNotFound_WhenIdIsInvalid()
        {
            // Arrange
            var admin = new AppUser { UserName = "admin", Email = "admin@site.com" };
            ...
            // Act
            var result = await _controller.ToggleRecipeVisibility(invalidId, "Show");
