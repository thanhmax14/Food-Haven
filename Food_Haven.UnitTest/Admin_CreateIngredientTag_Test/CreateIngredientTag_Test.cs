...
namespace Food_Haven.UnitTest.Admin_CreateIngredientTag_Test
{
    [TestFixture]
    public class CreateIngredientTag_Test
    {
        ...
        public void Setup()
        {
            ...
            _manageTransaction = manageTransactionMock.Object; // <-- Assign the mock object
        ...
        [Test]
        public async Task Should_Return_View_When_IngredientTagName_Exists()
        {
            var model = new IngredientTagViewModel { Name = "Test", IsActive = true };
            _ingredientTagServiceMock.Setup(x => x.ExistsAsync(model.Name)).ReturnsAsync(true);

            var result = await _controller.CreateIngredientTag(model);
        ...
        [Test]
        public async Task Should_Redirect_When_IngredientTag_Created_Successfully()
        {
            var model = new IngredientTagViewModel { Name = "Test", IsActive = true };
            _ingredientTagServiceMock.Setup(x => x.ExistsAsync(model.Name)).ReturnsAsync(false);
            _ingredientTagServiceMock.Setup(x => x.AddAsync(It.IsAny<IngredientTag>())).Returns(Task.CompletedTask);
            ...
            Assert.That(_controller.TempData["SuccessMessage"], Is.EqualTo("Ingredient tag has been created successfully!"));
        }
    }
}
